...
def search_project_live(project_id: int, query: str, target_language_id: str, limit: int) -> list[dict]:
    ...
    source_text, source_snippet, target_snippet} — file_path is added
    by the caller from the local files cache (already synced via
    tree_sync, no extra API call needed for that part)."""
    ...
    translations_by_string: dict[int, list[str]] = {}
    for item in translations_resp.get("data", []):
        t = _unwrap(item)
        translations_by_string.setdefault(t["stringId"], []).append(t["text"])
    ...
    for s in strings:
        ...
        # the target snippet is the relevant one when a string has
        # several contributor translations.
        target_text = next((t for t in candidates if literal.lower() in t.lower()), None)
        if target_text is None and candidates:
